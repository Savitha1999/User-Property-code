...
const handleHeartClick = async () => {
  ...
  try {
      ...
      if (status === "favorite") {
          ...
          setMessage("Your Favorite Removed");
          setPostedUserPhoneNumber("");
          localStorage.setItem(`isHeartClicked-${
            
            
            ppcId}`, JSON.stringify(false));
      } else if (status === "alreadySaved") {
        setMessage("You Have Already Favorited")
          setIsHeartClicked(true);
      }
  } catch (error) {
...
const handlePageNavigation = () => {
  ...
  return (
    ...
    >
      ...
      {showShareOptions && (
        <div
          className="d-flex flex-column gap-2 mt-4 p-3"
          style={{
            position: "absolute",
            top: "90px",
            right: "42%",
